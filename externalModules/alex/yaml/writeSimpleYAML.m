...
for jj=1:numel(s)
    ...
    for ii=1:numel(f)
        ...
        if isnumeric(currVal)
            fprintf(fid, '%s: %s', f{ii}, mat2str(currVal));
        elseif ischar(currVal)
            fprintf(fid, '%s: %s', f{ii}, currVal);
        elseif isstruct(currVal)
            fprintf(fid, sprintf('%s:\n', f{ii}));
            if isscalar(currVal)
                writeYamlEntry(fid,currVal, indentLevel+1+1)
            else
                writeYamlStructArrayEntry(fid, currVal, indentLevel+1+1)
            end
        elseif iscell(currVal)
            if all(cellfun(@isstr,currVal)) %hand a cell array of strings
                tmp=sprintf('%s, ',currVal{:});
                tmp(end-1:end)=[];
                tmp=['{',tmp,'}'];
                fprintf(fid,'%s: %s', f{ii}, tmp);
            else
